...
{
    /*
     * 发送命令必须建立在 UART 已初始化的基础上。
     */
    ESP_RETURN_ON_FALSE(s_ctx.inited, ESP_ERR_INVALID_STATE, TAG, "not initialized");

    /*
     * 先尝试把旧版单字符命令映射为新版协议命令码。
     * 能映射说明这个命令已经被新版二进制协议支持。
     */
    ...
    if (proto_cmd != APP_CH32_PROTO_CMD_NONE) {
         ...
         * 优先发送新版协议。
         *
         * 新协议有 CRC 和序号，更适合实际比赛/演示中的抗干扰需求。
         */
    ...
    /*
     * 如果命令没有新版映射，或新版发送失败，则回退旧文本协议。
     * 这样 ESP32 端可以兼容还没有升级协议的 CH32 固件。
     */
...
esp_err_t app_ch32_link_wait_ready(uint32_t timeout_ms)
{
    /*
     * 等待 ready 是启动阶段常用接口。
     * app_ctrl 或 main 链路可以用它确认 CH32 副控已经在线。
     */
    ESP_RETURN_ON_FALSE(s_ctx.inited, ESP_ERR_INVALID_STATE, TAG, "not initialized");

    /*
     * 如果 ready 已经被 RX 任务记录，直接成功。
     */
    if (s_ctx.ready) {
        // 正常返回 ESP_OK，表示该步骤执行成功。
        return ESP_OK;
    ...
    /*
     * 使用分片重试，而不是一次性阻塞 timeout_ms。
     *
     * 这样可以周期性发 probe 命令，适配 CH32 上电较慢、复位中、
     * 或第一条 UART 命令丢失的情况。
     */
    uint32_t elapsed_ms = 0;
    ...
    while (elapsed_ms < timeout_ms) {
        /*
         * 本轮 probe 最多等待 slice_ms，但最后一轮不能超过总 timeout。
         */
        uint32_t this_wait = slice_ms;
        if ((elapsed_ms + this_wait) > timeout_ms) {
            this_wait = timeout_ms - elapsed_ms;
    ...
    /*
     * 在总超时时间内没有等到 ready。
     * 上层通常会在 UI 上提示 CH32 offline，或允许系统继续以降级模式运行。
     */
    return ESP_ERR_TIMEOUT;
}
/*
